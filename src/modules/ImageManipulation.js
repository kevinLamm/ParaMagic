...
export function normalizeImageEntity(input = {}) {
  return {
    ...
    x: finite(input.x, 0),
    y: finite(input.y, 0),
    width: Math.max(minimumImageSize, Math.abs(finite(input.width, 240))),
    height: Math.max(minimumImageSize, Math.abs(finite(input.height, 160))),
...
export function createImageManipulation({ addSvg, parent, screenToWorld, getScale, evaluateNumeric, onSelect, onMoveStart, onChange, onDelete }) {
  let drag = null;
  ...
  function updateRecord(record) {
    ...
    record.toolbar.setAttribute('x', -51 / scale);
    record.toolbar.setAttribute('y', -visualTop - 68 / scale);
    record.toolbar.setAttribute('width', 102 / scale);
    record.toolbar.setAttribute('height', 34 / scale);
    record.toolbarContent.style.transform = `scale(${1 / scale})`;
    record.toolbarContent.style.transformOrigin = '0 0';
    record.toolbarContent.style.width = '102px';
  ...
  function createRecord(input) {
    ...
    toolbarContent.append(lockButton, horizontalButton, verticalButton);
    toolbar.appendChild(toolbarContent);
    const record = { id: entity.id, recordType: 'image', entity, group, transform, image, hitTarget, selectionFrame, handleGroup, handles: [...cornerHandles, rotationHandle], cornerHandles, rotationHandle, rotationStem, toolbar, toolbarContent, lockButton, horizontalButton, verticalButton };
